...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    if agent_cfg.resume or agent_cfg.algorithm.class_name == "Distillation":
        ...
        # Keep the algorithm's learning-rate scalar in sync with the restored optimizer state.
        if hasattr(runner.alg, "optimizer") and hasattr(runner.alg, "learning_rate"):
            param_groups = getattr(runner.alg.optimizer, "param_groups", None)
            if param_groups:
                runner.alg.learning_rate = param_groups[0]["lr"]
